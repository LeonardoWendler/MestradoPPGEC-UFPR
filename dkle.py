# -*- coding: utf-8 -*-
"""
Created on Sun Sep  8 22:35:58 2024
...
class DiscreteKarhunenLoeveExpansion(object):
    ...
    # The standard normal RV realizations
    _RV_realizations = None   
    ...
    def RV_realize(self):
        """
        Get the standard normal RV realizations
        """
        return self._RV_realizations

    # def __init__(self, points, eig_vectors, eig_values, mean_value, name='D-KLE'):
    def __init__(self, points, eig_vectors, eig_values, mean_value, RV_realizations, name='D-KLE'):        
        """
        ...
        self._RV_realizations = RV_realizations
    ...
    def __call__(self, xi):
        ...
        return self.mean_val + np.dot(xi * np.sqrt(self.eig_values), self.eig_vectors.T)

    def sample(self, size=1):
        """
        Sample random fields.
        """
        # For standard normal gaussian RV
        # xi = np.random.normal(0, 1, size=(size, self.num_terms))
        xi = self._RV_realizations.T  
        return self(xi)
    ...
    #         return np.array([self.project(X[x, :]) for x in range(X.shape[0])])
